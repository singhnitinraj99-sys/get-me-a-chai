      ...
      <div className="flex justify-center gap-8 mb-10 text-center">
        ...
        <div>
          <p className="text-2xl font-bold text-emerald-400">
            ₹
            {creators
              .reduce((sum, creator) => sum + (creator.totalAmount || 0), 0)
        ...
        <div>
          <p className="text-2xl font-bold text-blue-400">
            {creators.reduce(
              (sum, creator) => sum + (creator.totalCount || 0),
              0
            )}
          </p>
                ...
                <div className="mt-4 flex items-center justify-between border-t border-slate-800 pt-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-slate-400 text-xs">
                      {creator.totalCount || 0} chais
                    </span>
                  </div>

                  <div>
                    {(creator.totalAmount || 0) > 0 ? (
                      <span className="text-emerald-400 font-semibold text-sm">
                        ₹{(creator.totalAmount || 0).toLocaleString("en-IN")}
                      </span>
                    ) : (
